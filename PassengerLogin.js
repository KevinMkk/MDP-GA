...
import { loginPassenger } from './apiService';

const PassengerLogin = ({ navigation }) => {
  ...
  const handleLogin = async () => {
    try {
      ...
      if (result && result.data) {
        navigation.navigate('Passenger Menu', { passenger: result.data });
      } else {
        Alert.alert('Login Failed', 'Please check your email and password and try again.');
      }
    } catch (error) {
      Alert.alert('Error', 'Error logging in passenger');
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.signUpText}>
        No account? Sign-Up 
        <TouchableOpacity onPress={() => navigation.navigate("Passenger Signup")}>
